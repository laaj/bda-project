...
animate(p + transition_time(iter), nframes = nframes, fps = 1)
```
...
Let's then look at some of the MCMC-specific convergence metrics by using the $\texttt{monitor}$ function from $\texttt{rstan}$.
